...
import { generateCoverLetterApi, translatePersonalStatementToOriginalLanguageApi } from "../../../api/caseAPI";
...
export function CoverLetter(props: CoverLetterProps) {
  ...
  useEffect(() => {
    ...
    if (props.value === "") {
      ...
      try {
        const cl = props.value;
        setValue(cl);
      } catch (error) {
        setValue("");
        message.error("Failed to display the stored cover letter.");
      }
    }
  }, [props.value]);
  ...
  const generateCoverLetter = async () => {
    ...
    try {
      ...
      const cl = await generateCoverLetterApi(accessToken, role, caseId);

      setValue(cl);
      setIsOriginalLoading(false);
      props.onChange(cl);
      //   savePersonalStatement(cl, translatedValue);
    } catch (error) {
      setIsOriginalLoading(false);
      message.error("Failed to generate cover letter. Please try again.");
    }
  };
              ...
              <Tooltip
                title={
                  !isAllPreviousSectionComplete
                    ...
                    : t("AI will generate the Cover Letter based on the information provided in the previous sections.")
                }
              >
                <Button
                  type="primary"
                  ...
                  icon={<AiIcon />}
                >
                  {t("Write your Cover Letter with AI")}
                </Button>
              </Tooltip>
            }
            {isOriginalLoading ? (
